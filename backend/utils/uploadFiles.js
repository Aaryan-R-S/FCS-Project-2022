...
const storage = multer.diskStorage({
    ...
    filename: function(req, file, cb) {
        let filename = req.body.licenseno + randomFileName(32) + Date.now() ;      
...
const upload = multer({ storage: storage, fileFilter: multerFilter, limits: {fileSize: 1024 * 1024} }).array('files', 5);
